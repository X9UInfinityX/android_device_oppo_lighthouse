...
constexpr char kPanelRefreshRatePath[] = "/sys/kernel/oplus_display/test_te";
...
constexpr auto kMinFpsMirrorInterval = std::chrono::milliseconds(250);
...
constexpr int kHighestUserMinFps = 144;
...
bool enableOplusAutoQsync() {
    ndk::SpAIBinder binder(AServiceManager_checkService(kDisplayConfigServiceName));
    ...
    // Stock F9UEU SDM interprets 0x5c as the OPlus min-fps command and
    // 0xc18181 as automatic Qsync. Keep the standard, versioned AIDL transport.
    const auto status = displayConfig->setQsyncMode(
            kPrimaryDisplay, static_cast<QsyncMode>(kOplusAutoQsyncMode));
    if (!status.isOk()) {
        LOG(ERROR) << "Failed to enable OPlus auto Qsync: " << status.getDescription();
        return false;
    }
    return true;
}
...
bool parseAdfrConfig(AdfrConfig* config) {
    ...
    // F9UEU consumes fixed slots in a 225-word payload. Reject lists that
    // would overlap another slot, and incomplete brightness/gray tables.
    auto validTable = [](const auto& nit, const auto& low, const auto& high,
                         const auto& sum, const auto& max, const auto& fps120,
                         const auto& fps90, const auto& fps60, bool required) {
        if (nit.empty()) {
            return !required && low.empty() && high.empty() && sum.empty() &&
                    max.empty() && fps120.empty() && fps90.empty() && fps60.empty();
        }
        const size_t n = nit.size();
        const size_t fpsCount = 2 * (n + 1);
        const auto validFps = [fpsCount](const auto& fps) {
            return fps.size() == fpsCount &&
                    std::all_of(fps.begin(), fps.end(), [](int value) {
                        return value >= 1 && value <= kHighestUserMinFps;
                    });
        };
        return n <= 5 && low.size() == n && high.size() == n && sum.size() == n &&
                max.size() == n && validFps(fps120) && validFps(fps90) && validFps(fps60);
    };
    if (!validTable(config->panelNitLevel, config->yLLevel, config->yHLevel,
                    config->sumLevel, config->maxLevel, config->minFps120Level,
                    config->minFps90Level, config->minFps60Level, true) ||
        !validTable(config->aodPanelNitLevel, config->aodYLLevel, config->aodYHLevel,
                    config->aodSumLevel, config->aodMaxLevel, config->aodMinFps120Level,
                    config->aodMinFps90Level, config->aodMinFps60Level, true) ||
        !validTable(config->reservePanelNitLevel, config->reserveYLLevel,
                    config->reserveYHLevel, config->reserveSumLevel, config->reserveMaxLevel,
                    config->reserveMinFpsLevel1, config->reserveMinFpsLevel2,
                    config->reserveMinFpsLevel3, config->reserveMode != 0)) {
        LOG(ERROR) << "Invalid ADFR brightness/gray table dimensions or FPS values";
        return false;
    }
...
int readUserMinFpsFloor() {
    ...
    if (android::base::GetBoolProperty(kBootCompletedProperty, false) &&
        minFps >= kLowestUserMinFps && minFps <= kHighestUserMinFps) {
        return minFps;
    }
...
void mirrorMinFpsProperty(const ndk::SpAIBinder& panelBinder) {
    std::string lastValue;
    int lastPowerStatus = readPanelPowerStatus();
    int lastMinFpsFloor = readUserMinFpsFloor();
    auto floorQsyncRetryAfter = std::chrono::steady_clock::time_point::min();
    ...
    while (AIBinder_isAlive(panelBinder.get())) {
        ...
        if (powerStatus == 0) {
            // A floor is normally only raised to preserve stock brightness safeguards.
            // When the user lowers it, release the old request through stock Qsync
            // before applying the new floor, including zero (automatic LTPO).
            if (minFpsFloor < lastMinFpsFloor && now >= floorQsyncRetryAfter) {
                if (enableOplusAutoQsync()) {
                    LOG(INFO) << "Released Oplus min fps floor " << lastMinFpsFloor
                              << " for " << minFpsFloor;
                    lastMinFpsFloor = minFpsFloor;
                }
                floorQsyncRetryAfter = now + kQsyncRetryDelay;
            } else if (minFpsFloor >= lastMinFpsFloor) {
                lastMinFpsFloor = minFpsFloor;
                floorQsyncRetryAfter = std::chrono::steady_clock::time_point::min();
            }
        ...
        // X9 Ultra uses the kernel software marker for test_te (ADFR bit 14).
        // Publish that driver estimate, not rendered FPS, and clear it while off.
        const int refreshRate = powerStatus == 0 ? readIntFile(kPanelRefreshRatePath) : 0;
        if (refreshRate >= 0 && refreshRate <= kHighestUserMinFps) {
            const std::string value = std::to_string(refreshRate);
            if (value != lastValue && android::base::SetProperty(kOplusRefreshRateProperty, value)) {
                lastValue = value;
            }
        }
...
int main() {
    ...
    mirrorMinFpsProperty(binder);
    LOG(ERROR) << "Panel service died; restarting ADFR initialization";
    return 1;
}
